"""
DHL Document Intelligence — Gradio App (V2)
Capgemini Theme | Fine-tuned V2 Model | Classification + Extraction + Splitting
...
MERGED_PATH  = BASE_DIR / "model_output_v2" / "merged"
MAX_PIXELS = 384_000   # ~490 visual tokens — faster vision encoding (~45% fewer tokens vs 640K)
FIRST_PREV = "none (first page of batch)"
MODEL_DISPLAY_NAME = "FFDoc_3B"

# Load Capgemini logo at startup — encode as base64 from the project file.
# Using SVG <image> in the header (not HTML <img>) so Gradio 5.x CSS resets
# for img elements don't interfere.
_CAP_LOGO_B64 = ""
try:
    import base64 as _b64, io as _io
    _logo_file = BASE_DIR / "capgemini_logo.jpg"
    if _logo_file.exists():
        _img_pil = Image.open(str(_logo_file)).convert("RGB")
        _lw, _lh = _img_pil.size
        _target_h = 104           # 2× display density for crisp rendering
        _target_w = int(_lw * _target_h / _lh)
        _img_pil = _img_pil.resize((_target_w, _target_h), Image.LANCZOS)
        _buf = _io.BytesIO()
        _img_pil.save(_buf, format="JPEG", quality=85)
        _CAP_LOGO_B64 = _b64.b64encode(_buf.getvalue()).decode()
        del _img_pil, _buf, _lw, _lh, _target_h, _target_w
    del _b64, _io, _logo_file
except Exception as _e:
    print(f"  [Logo] Could not load capgemini_logo.jpg: {_e}")
...
/* Prevent Gradio from collapsing header logo */
.gradio-html img { max-width: none !important; height: auto; }
.upload-zone {
...
def build_app():
    ...
    # Build logo element: SVG <image> bypasses the global img{} CSS reset in Gradio 5.x
    if _CAP_LOGO_B64:
        _logo_el = (
            f'<svg xmlns="http://www.w3.org/2000/svg" width="160" height="52"'
            f' style="display:block;flex-shrink:0;border-radius:8px;overflow:hidden;">'
            f'<image href="data:image/jpeg;base64,{_CAP_LOGO_B64}"'
            f' x="0" y="0" width="160" height="52"'
        ...
        _logo_el = (
            ...
            'cap<span style="color:#00B1C1">gemini</span></span>'
        )

    with gr.Blocks(css=CSS, title="Freight Forward Doc Intelligence | FFDoc_3B") as app:
        ...
        <div style="background:linear-gradient(135deg,#002E5E 0%,#003F7D 55%,#0070AD 100%);
          ...
          <!-- Left: title + model tag -->
          <div>
            <div style="font-size:21px;font-weight:800;color:white;letter-spacing:-0.4px;
                        line-height:1.25;font-family:'Segoe UI',Arial,sans-serif">
              Freight Forward Doc Intelligence
            </div>
            <div style="margin-top:10px;display:flex;align-items:center;gap:8px">
              <span style="font-size:11px;color:rgba(255,255,255,0.55);font-weight:400;
                           letter-spacing:0.15px;font-family:'Segoe UI',Arial,sans-serif">
                Powered by
              </span>
              <span style="background:rgba(0,177,193,0.15);border:1px solid rgba(0,229,245,0.45);
                           border-radius:5px;padding:3px 11px;font-size:11.5px;font-weight:700;
                           color:#00E5F5;letter-spacing:1px;font-family:'Courier New',monospace">
                FFDoc_3B
              </span>
            </div>
          </div>

          <!-- Right: Capgemini logo via SVG <image> (not HTML <img>).
               SVG elements are exempt from the global img{{}} CSS reset in Gradio 5.x,
               so explicit width/height attributes on <svg> are honoured. -->
          {_logo_el}
        ...
        with gr.Row():
            with gr.Column(scale=3):
                gr.HTML("""
                <div style="font-size:13px;font-weight:600;color:#003F7D;margin-bottom:8px;
                            display:flex;align-items:center;gap:8px">
                  📄 Upload Document(s)
                  <span onclick="(function(){var p=document.getElementById('sup-fmt-popup');p.style.display=p.style.display==='none'?'block':'none'})()"
                        title="View supported formats"
                        style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:50%;
                               width:19px;height:19px;display:inline-flex;align-items:center;
                               justify-content:center;font-size:11px;cursor:pointer;
                               color:#0070AD;flex-shrink:0;user-select:none;font-style:italic;
                               font-weight:700;line-height:1">i</span>
                </div>
                <div id="sup-fmt-popup" style="display:none;margin-bottom:10px">
                  <div style="background:white;border:1px solid #e2e8f0;border-radius:10px;
                              padding:16px 18px;box-shadow:0 8px 24px rgba(0,0,0,0.10);
                              font-family:'Segoe UI',Arial,sans-serif;max-width:460px">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
                      <div style="font-size:11px;font-weight:700;color:#003F7D;letter-spacing:0.6px">
                        SUPPORTED INPUT FORMATS
                      </div>
                      <span onclick="document.getElementById('sup-fmt-popup').style.display='none'"
                            style="cursor:pointer;color:#94a3b8;font-size:14px;padding:2px 5px;
                    ...
                    <div style="display:grid;gap:8px">
                      <div style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;
                                  background:#f8fafc;border-radius:8px;border-left:3px solid #0070AD">
                        <div style="font-size:16px;flex-shrink:0;margin-top:1px">📄</div>
                        <div>
                          <div style="font-size:12px;font-weight:700;color:#1e293b">PDF Documents</div>
                          <div style="font-size:11px;color:#64748b;margin-top:2px">
                            Digital or scanned — both fully supported
                          </div>
                        </div>
                      </div>
                      <div style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;
                                  background:#f8fafc;border-radius:8px;border-left:3px solid #00B1C1">
                        <div style="font-size:16px;flex-shrink:0;margin-top:1px">🖼️</div>
                        <div>
                          <div style="font-size:12px;font-weight:700;color:#1e293b">Images</div>
                          <div style="font-size:11px;color:#64748b;margin-top:2px">
                            JPG, PNG, TIFF, BMP, WebP
                          </div>
                          <div style="font-size:11px;color:#64748b;margin-top:2px">
                            Upload multiple images together for multi-page documents
                          </div>
                        </div>
                      </div>
                      <div style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;
                                  background:#f8fafc;border-radius:8px;border-left:3px solid #f59e0b">
                        <div style="font-size:16px;flex-shrink:0;margin-top:1px">✨</div>
                        <div>
                          <div style="font-size:12px;font-weight:700;color:#1e293b">Auto-Enhancement</div>
                          <div style="font-size:11px;color:#64748b;margin-top:2px">
                            Photos &amp; screenshots are automatically enhanced before analysis
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div style="text-align:center;color:#94a3b8;font-size:11px;
          ...
          capgemini &nbsp;·&nbsp; Freight Forward Doc Intelligence &nbsp;·&nbsp; FFDoc_3B
        </div>
        """)
...
if __name__ == "__main__":
    print("=" * 60)
    print("  DHL Document Intelligence — Capgemini (V2)")
    print("=" * 60)
