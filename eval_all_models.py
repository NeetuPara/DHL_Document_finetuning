...
# Speed: TF32 on Ampere/Ada/Hopper GPUs (RTX 5080, H100) — no accuracy loss for inference
...
def is_blank_doc(label: dict) -> bool:
    """True if all START_FIELDS are null — blank/empty form example."""
    if label.get("position") != "START":
        return False
    return all(
        label.get(f) is None or str(label.get(f) or "").strip() == ""
...
# InternVL2 prompt — same fields but shorter/more direct for a 2B zero-shot model
# InternVL2 uses its own chat template internally; simpler prompt yields better compliance
INTERNVL2_PROMPT = (
    "Look at this logistics document image carefully.\n\n"
    "Previous page context: {prev}\n\n"
    "Return a single JSON object with these exact keys:\n"
    '  {{"class": "<document type>", "position": "START or CONTINUATION", '
    '"shipper_name": null, "consignee_name": null, "document_date": null, '
    '"document_number": null, "country_of_origin": null, '
    '"country_of_destination": null, "description_of_goods": null, '
    '"license_number": null, "validity_start": null, '
    '"validity_end": null, "licensee_name": null}}\n\n'
    "Rules:\n"
    "- class must be one of: " + ", ".join(CLASSES) + "\n"
    "- position: START if this begins a new document, CONTINUATION if it continues the previous\n"
    "- Fill each field with the exact text from the document, or null if not present\n"
    "- If position is CONTINUATION, only output class and position\n"
    "- Never guess or invent values — use null for anything not clearly visible"
)

# Donut-RVLCDIP class map — its 16 training classes → our closest DHL class
# Used for fuzzy matching when Donut outputs one of its known class labels
DONUT_RVL_CLASS_MAP = {
    "invoice":                  "Commercial Invoice",
    "advertisement":            "Commercial Invoice",
    "budget":                   "Commercial Invoice",
    "form":                     "Shipper's Letter of Instruction",
    "questionnaire":            "Shipper's Letter of Instruction",
    "specification":            "Import/Export License",
    "memo":                     "Power of Attorney",
    "letter":                   "Power of Attorney",
    "scientific_report":        "Certificate of Origin",
    "scientific_publication":   "Certificate of Origin",
    "file_folder":              "Packing List",
    "presentation":             "Cargo Manifest",
    "news_article":             "Cargo Manifest",
    "email":                    "House Bill of Lading",
    "handwritten":              "Dangerous Goods Declaration",
    "resume":                   "House Airway Bill",
}
...
def _find_our_checkpoint(base_dir: Path) -> Path | None:
    """Prefer model_output_v3/final; fall back to highest numbered checkpoint."""
...
def _ours_cfg(lora_path, merged_path, base_path, label, short_label):
    """Return registry entry: merged (fast) if available, else runtime LoRA merge."""
    if Path(merged_path).exists():
        return {
            "label": label + "  [merged]", "short_label": short_label,
            "type": "qwen", "path": str(merged_path), "metrics": "all",
        }
    lp = Path(lora_path)
    if lp.exists():
        return {
            "label": label + "  [LoRA]", "short_label": short_label,
            "type": "ours", "path": str(lp), "base": str(base_path), "metrics": "all",
        }
    return {
        "label": label, "short_label": short_label,
        "type": "ours", "path": None, "base": str(base_path), "metrics": "all",
    }


def build_model_registry(base_dir: Path, our_checkpoint: str | None) -> dict:
    our_lora = Path(our_checkpoint) if our_checkpoint else _find_our_checkpoint(base_dir)
    if our_lora and not our_lora.is_absolute():
        our_lora = base_dir / our_lora
    base_model = base_dir / "models" / "Qwen2.5-VL-3B-Instruct"
    ...
    return {
        ...
        "qwen7b": {
            "label":       "Qwen2.5-VL-7B  |  Zero-Shot  |  Full Precision  |  H100",
            "short_label": "Qwen-7B-ZS",
            "type":        "qwen",
            "path":        str(base_dir / "models" / "Qwen2.5-VL-7B-Instruct"),
            "metrics":     "all",
            "min_vram_gb": 40,      # deferred: skip on <40 GB, placeholder column in Excel
        },
        "internvl2": {
        ...
        "donut": {
            "label":       "Donut-RVLCDIP  |  Zero-Shot  |  Document Classification Specialist",
            "short_label": "Donut-RVL",
            "type":        "donut",
            "path":        "naver-clova-ix/donut-base-finetuned-rvlcdip",
            "metrics":     "class_only",   # RVLCDIP: classification only, no position/field
        },
        "ours": _ours_cfg(
            lora_path   = our_lora,
            merged_path = base_dir / "model_output_v3" / "merged",
            base_path   = base_model,
            label       = f"Ours: Qwen2.5-VL-3B  |  Fine-Tuned v3  |  {our_lora.name if our_lora else 'NOT FOUND'}",
            short_label = "Ours-V3",
        ),
        "ours_ck2000": _ours_cfg(
            lora_path   = base_dir / "model_output_v3" / "checkpoint-2000",
            merged_path = base_dir / "model_output_v3" / "ck2000_merged",
            base_path   = base_model,
            label       = "Ours: Qwen2.5-VL-3B  |  Fine-Tuned v3  |  checkpoint-2000",
            short_label = "Ours-CK2000",
        ),
        "ours_ck2250": _ours_cfg(
            lora_path   = base_dir / "model_output_v3" / "checkpoint-2250",
            merged_path = base_dir / "model_output_v3" / "ck2250_merged",
            base_path   = base_model,
            label       = "Ours: Qwen2.5-VL-3B  |  Fine-Tuned v3  |  checkpoint-2250",
            short_label = "Ours-CK2250",
        ),
        "oursv2": {
            "label":       "Ours: Qwen2.5-VL-3B  |  Fine-Tuned v2  |  Merged",
            "short_label": "Ours-V2",
            "type":        "qwen",
            "path":        str(base_dir / "model_output_v2" / "merged"),
            "metrics":     "all",
        },
    }
...
def parse_args():
    ...
    ALL_MODELS     = ["qwen3b", "qwen7b", "internvl2", "donut", "ours", "ours_ck2000", "ours_ck2250", "oursv2"]
    DEFAULT_MODELS = ["qwen3b", "qwen7b", "donut", "ours", "ours_ck2000", "ours_ck2250", "oursv2"]
    # qwen7b: deferred on <40 GB GPU — placeholder column in Excel, fill later on H100
    # internvl2: excluded — transformers version conflict; run on H100
    # oursv3merged: excluded — requires pre-merge (python merge_model.py)
    p.add_argument("--models", nargs="+",
                   choices=ALL_MODELS,
                   default=DEFAULT_MODELS,
                   help="Which models to evaluate (default: all except qwen7b — add manually if H100 available)")
    p.add_argument("--our-checkpoint", default=None,
                   help="Path to fine-tuned v3 checkpoint (default: auto-detect latest)")
    p.add_argument("--test-jsonl",  default="Training_Data_v3/test_new.jsonl")
    p.add_argument("--image-base",  default="Training_Data")
    p.add_argument("--max-samples", type=int, default=0,
                   help="Max samples per model, stratified (0=all — default)")
...
def load_qwen(model_path: str, device: str):
    ...
    try:
        model = MC.from_pretrained(model_path, attn_implementation="sdpa", **kwargs)
        print("  [sdpa attention enabled]")
    except (ValueError, NotImplementedError):
        model = MC.from_pretrained(model_path, **kwargs)
    model.eval()
...
def load_internvl2(model_path: str, device: str):
    ...
    # InternVL2 ViT __init__ calls torch.linspace(...).item(). In PyTorch 2.7+
    # a TorchDispatchMode active during loading routes linspace through meta
    # dispatch, making .item() crash. Monkey-patch linspace to force CPU tensors
    # during model construction, then restore immediately after.
    _orig_linspace = torch.linspace
    torch.linspace = lambda *a, **kw: _orig_linspace(*a, **{**kw, "device": "cpu"})
    try:
...
def load_ours(lora_path: str, base_path: str, device: str):
    ...
    kwargs = dict(torch_dtype=torch.bfloat16, device_map=device)
    try:
        base = MC.from_pretrained(base_path, attn_implementation="sdpa", **kwargs)
        print("  [sdpa attention enabled]")
    except (ValueError, NotImplementedError):
        base = MC.from_pretrained(base_path, **kwargs)
...
def unload(model):
    ...
    del model
    _QWEN_TMPL_CACHE = None   # reset between models so next model's processor builds fresh
...
def infer_internvl2(model, tokenizer, image, prev_label, max_new_tokens):
    ...
    with torch.inference_mode():
        response = model.chat(tokenizer, pixel_values, INTERNVL2_PROMPT.format(prev=prev_label), gen_cfg)
...
def infer_donut(model, processor, image, device):
    """
    Donut-RVLCDIP: outputs one of 16 document classes via <s_rvlcdip> task prefix.
    We map its 16 classes to our nearest DHL class, position always returns START
    (RVLCDIP doesn't model page position — class_only metrics applies).
    """
    ...
    # Extract class label: <s_class>invoice</s_class>
    m = re.search(r"<s_class>(.*?)</s_class>", seq, re.DOTALL)
    rvl_label = m.group(1).strip().lower() if m else seq.strip().lower()

    # Map RVL-CDIP label → DHL class
    matched_cls = DONUT_RVL_CLASS_MAP.get(rvl_label)
    if not matched_cls:
        # Fuzzy fallback: check if any DHL class name appears
        for c in sorted(CLASS_SET, key=len, reverse=True):
            if c.lower() in rvl_label:
                matched_cls = c
                break

    return json.dumps({"class": matched_cls, "position": "START"})
...
def evaluate_model(model_key: str, cfg: dict, samples: list,
    ...
    # ── Deferred model: skip on low-VRAM GPU, insert empty placeholder column ─
    min_vram = cfg.get("min_vram_gb", 0)
    if min_vram > 0:
        vram_gb = torch.cuda.get_device_properties(0).total_memory / 1e9 \
                  if torch.cuda.is_available() else 0
        if vram_gb < min_vram:
            print(f"  Deferring {model_key} — needs {min_vram:.0f}+ GB, have {vram_gb:.0f} GB")
            print(f"  Column reserved in Excel. Run on H100 with --models {model_key} --resume to fill.")
            return {
                "model":             cfg["label"],
                "short_label":       cfg.get("short_label", model_key),
                "metrics_type":      cfg.get("metrics", "all"),
                "n": 0, "n_filled": 0, "n_blank": 0,
                "class_accuracy": None, "position_accuracy": None,
                "field_f1": None, "split_iou": None,
                "per_class": {}, "per_field_f1": {f: None for f in START_FIELDS},
                "per_class_field_f1": {cls: None for cls in CLASSES},
                "blank_metrics": {}, "split_complexity": {},
                "deferred": True,
            }

    # ── Partial resume: load any existing progress ────────────────────────────
    partial_results = []
    if results_path.exists():
        try:
            partial_results = json.loads(results_path.read_text())
        except Exception:
            partial_results = []

    if partial_results and len(partial_results) >= len(samples):
        print(f"  Skipping {model_key} — fully complete ({len(partial_results)} samples)")
        return compute_metrics(partial_results, cfg["label"], cfg["metrics"], cfg.get("short_label", ""))

    if partial_results:
        print(f"  Resuming {model_key} — {len(partial_results)}/{len(samples)} already done, continuing...")
    ...
    try:
        ...
        print(f"  Skipping {model_key} — load failed.")
        torch.cuda.empty_cache()
        return {"model": cfg["label"], "short_label": cfg.get("short_label", model_key), "error": str(e)}

    # ── Inference loop ────────────────────────────────────────────────────────
    from PIL import Image
    results   = list(partial_results)          # start from any existing progress
    remaining = samples[len(partial_results):]  # skip already-done samples
    t_start   = time.time()
    ...
    for i, sample in enumerate(remaining):
        ...
        # Blank doc: detect which fields the model hallucinated
        is_blank = is_blank_doc(true_label)
        hallucinated_fields = []
        if is_blank and true_pos == "START":
            hallucinated_fields = [
                f for f in START_FIELDS
                if pred.get(f) is not None and str(pred.get(f) or "").strip() != ""
            ]
        ...
        print(f"  [{len(results):>4}/{len(samples)}] "
        ...
        # Save every 25 samples — allows resume from mid-model if interrupted
        if len(results) % 25 == 0:
...
def compute_metrics(results: list, label: str, metrics_type: str, short_label: str = "") -> dict:
    ...
    # ── Separate blank docs — excluded from main accuracy metrics ─────────────
    blank_results  = [r for r in results if r.get("is_blank_doc")]
    filled_results = [r for r in results if not r.get("is_blank_doc")]
    eval_r = filled_results if filled_results else results  # main metrics on filled only

    # ── Overall accuracy (filled docs only) ───────────────────────────────────
    class_acc = sum(r["class_correct"] for r in eval_r) / len(eval_r) * 100
    pos_acc   = sum(r["pos_correct"]   for r in eval_r) / len(eval_r) * 100

    # ── Per-class breakdown (filled only) ─────────────────────────────────────
    ...
    # ── Field F1 (filled START pages only) ───────────────────────────────────
    starts = [r for r in eval_r if r["is_start"]]
    ...
    # ── Split IoU + complexity breakdown (filled only) ───────────────────────
    # Use ALL results (including blank) for packet size — we need true page count
    all_packets = defaultdict(list)
    for r in results:
        m = re.search(r"(packet_\d+)", r["image"])
        all_packets[m.group(1) if m else r["image"]].append(r)

    # Filled-only packets for IoU
    ...
    if metrics_type == "all":
        ...
        # Group pages by their packet's true size, then measure pos/cls accuracy
        BUCKETS = {"1-page": (1, 1), "2-page": (2, 2),
                   "3-5 pages": (3, 5), "6+ pages": (6, 9999)}
        bucket_pages = {k: [] for k in BUCKETS}
        for pkt_name, pages in all_packets.items():
            n = len(pages)
            for bname, (lo, hi) in BUCKETS.items():  # bname avoids shadowing `label` param
                if lo <= n <= hi:
                    bucket_pages[bname].extend(
                        r for r in pages if not r.get("is_blank_doc"))
                    break
        split_complexity = {}
        for bname, bpages in bucket_pages.items():
            if bpages:
                split_complexity[bname] = {
                    "n":       len(bpages),
                    "pos_acc": round(sum(r["pos_correct"]   for r in bpages) / len(bpages) * 100, 1),
                    "cls_acc": round(sum(r["class_correct"] for r in bpages) / len(bpages) * 100, 1),
                }
            else:
                split_complexity[bname] = {"n": 0, "pos_acc": None, "cls_acc": None}
    else:
        split_iou        = None
        split_complexity = {}

    # ── Per-class field F1 (filled only) ─────────────────────────────────────
    per_class_field_f1 = {}
    if metrics_type == "all":
        for cls in CLASSES:
            cls_starts = [r for r in eval_r if r["is_start"] and r["true_class"] == cls]
            if cls_starts:
    ...
    # ── Blank doc metrics (hallucination tracking) ────────────────────────────
    blank_metrics = {}
    if blank_results:
        b_cls_acc    = sum(r["class_correct"] for r in blank_results) / len(blank_results) * 100
        b_hall_rate  = sum(1 for r in blank_results if r.get("hallucinated_fields")) / len(blank_results) * 100
        b_avg_fields = sum(len(r.get("hallucinated_fields", [])) for r in blank_results) / len(blank_results)
        blank_metrics = {
            "n":                       len(blank_results),
            "class_acc":               round(b_cls_acc, 1),
            "hallucination_rate":      round(b_hall_rate, 1),
            "clean_rate":              round(100 - b_hall_rate, 1),
            "avg_hallucinated_fields": round(b_avg_fields, 2),
            "per_field_hallucination": {
                f: round(
                    sum(1 for r in blank_results if f in r.get("hallucinated_fields", []))
                    / len(blank_results) * 100, 1
                )
                for f in START_FIELDS
            },
        }
...
def export_to_excel(all_metrics: list, out_path: Path):
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Sheet 7: Split Complexity (accuracy by document length)
    # ─────────────────────────────────────────────────────────────────────────
    COMPLEXITY_BUCKETS = ["1-page", "2-page", "3-5 pages", "6+ pages"]
    ws7 = wb.create_sheet("Split Complexity")
    ws7.row_dimensions[1].height = 50
    ws7.row_dimensions[2].height = 30

    note_sc = ws7.cell(row=1, column=1,
        value="Position accuracy and classification accuracy grouped by document length "
              "(number of pages in the original packet). Reveals whether models struggle "
              "more on longer/more complex bundles.")
    note_sc.font      = Font(italic=True, name="Calibri", size=10, color="555555")
    note_sc.alignment = LEFT
    ws7.merge_cells(start_row=1, start_column=1, end_row=1, end_column=N * 2 + 1)

    hdr(ws7, 2, 1, "Doc Length")
    ws7.merge_cells(start_row=2, start_column=1, end_row=3, end_column=1)
    col7 = 2
    model_col_map7 = {}
    for m, is_ours in zip(all_metrics, ours_flags):
        ws7.merge_cells(start_row=2, start_column=col7, end_row=2, end_column=col7 + 1)
        mc7 = ws7.cell(row=2, column=col7, value=m.get("short_label", m["model"][:15]))
        mc7.alignment = CENTER; mc7.border = BORDER
        mc7.fill = (PatternFill("solid", fgColor="1B5E20") if is_ours else NAVY_FILL)
        mc7.font = WHITE_BOLD
        for sub, sub_lbl in [(col7, "Pos Acc"), (col7 + 1, "Cls Acc")]:
            c7 = ws7.cell(row=3, column=sub, value=sub_lbl)
            c7.fill = NAVY_FILL; c7.font = WHITE_BOLD
            c7.alignment = CENTER; c7.border = BORDER
        model_col_map7[m["model"]] = col7
        col7 += 2

    for ri, bucket in enumerate(COMPLEXITY_BUCKETS, start=4):
        label_cell(ws7, ri, 1, bucket)
        for m, is_ours in zip(all_metrics, ours_flags):
            c = model_col_map7[m["model"]]
            sc = m.get("split_complexity", {}).get(bucket, {})
            val_cell(ws7, ri, c,     sc.get("pos_acc"), pct=True, ours=is_ours)
            val_cell(ws7, ri, c + 1, sc.get("cls_acc"), pct=True, ours=is_ours)

    # Sample counts per bucket (use first model — same for all)
    cnt_row = 4 + len(COMPLEXITY_BUCKETS)
    label_cell(ws7, cnt_row, 1, "Samples (n)")
    first_m = all_metrics[0] if all_metrics else {}
    for ri, bucket in enumerate(COMPLEXITY_BUCKETS):
        sc = first_m.get("split_complexity", {}).get(bucket, {})
        n_val = sc.get("n", 0)
        c_n = ws7.cell(row=4 + ri, column=1)
        c_n.value = f"{bucket}  (n={n_val})"
        c_n.fill = GREY_FILL; c_n.font = HEAD_FONT
        c_n.alignment = LEFT; c_n.border = BORDER

    ws7.freeze_panes = ws7.cell(row=4, column=2)
    autofit_cols(ws7)

    # ─────────────────────────────────────────────────────────────────────────
    # Sheet 8: Blank Documents (hallucination tracking)
    # ─────────────────────────────────────────────────────────────────────────
    ws7 = wb.create_sheet("Blank Documents")
    ws7.row_dimensions[1].height = 35

    note_b = ws7.cell(row=1, column=1,
        value="Blank/Empty Forms — EXCLUDED from main accuracy metrics above. "
              "Measures hallucination: did model correctly output all-null, or invent values?")
    note_b.font      = Font(italic=True, name="Calibri", size=10, color="555555")
    note_b.alignment = LEFT
    ws7.merge_cells(start_row=1, start_column=1, end_row=1, end_column=N + 1)

    hdr(ws7, 2, 1, "Metric")
    for i, (m, is_ours) in enumerate(zip(all_metrics, ours_flags)):
        c = ws7.cell(row=2, column=i + 2, value=m.get("short_label", m["model"][:15]))
        c.alignment = CENTER; c.border = BORDER
        c.fill = (PatternFill("solid", fgColor="1B5E20") if is_ours else NAVY_FILL)
        c.font = WHITE_BOLD
    ...
    BLANK_ROWS = [
        ("Blank Docs Evaluated (n)",       "n",                       False),
        ("Classification Accuracy",        "class_acc",               True),
        ("Clean Rate (all-null correct)",  "clean_rate",              True),
        ("Hallucination Rate",             "hallucination_rate",      True),
        ("Avg. Fields Hallucinated / doc", "avg_hallucinated_fields", False),
    ]
    for ri, (name, key, is_pct) in enumerate(BLANK_ROWS, start=3):
        label_cell(ws7, ri, 1, name)
        for ci, (m, is_ours) in enumerate(zip(all_metrics, ours_flags)):
            bm = m.get("blank_metrics", {})
            val_cell(ws7, ri, ci + 2, bm.get(key), pct=is_pct, ours=is_ours)

    # Per-field hallucination rates
    sep_row = 3 + len(BLANK_ROWS)
    sep_c = ws7.cell(row=sep_row, column=1,
        value="Per-Field Hallucination Rate (% of blank docs where field was wrongly filled)")
    sep_c.font = Font(bold=True, name="Calibri", size=10, color=NAVY_HEX)
    sep_c.fill = GREY_FILL; sep_c.alignment = LEFT; sep_c.border = BORDER
    ws7.merge_cells(start_row=sep_row, start_column=1, end_row=sep_row, end_column=N + 1)

    for fi, f in enumerate(START_FIELDS, start=sep_row + 1):
        label_cell(ws7, fi, 1, FIELD_DISPLAY_NAMES.get(f, f))
        for ci, (m, is_ours) in enumerate(zip(all_metrics, ours_flags)):
            bm = m.get("blank_metrics", {})
            v  = bm.get("per_field_hallucination", {}).get(f)
            val_cell(ws7, fi, ci + 2, v, pct=True, ours=is_ours)

    ws7.freeze_panes = ws7.cell(row=3, column=2)
    autofit_cols(ws7)
...
def print_comparison_table(all_metrics: list):
    print(f"\n{'='*90}")
    print("  COMPARISON TABLE  (main metrics on FILLED docs only — blank docs excluded)")
    print(f"{'='*90}")
    header = (f"  {'Model (short)':<18} {'Total':>6} {'Filled':>7} {'Blank':>6} "
    ...
    for m in all_metrics:
        ...
        bm = m.get("blank_metrics", {})
        hall = bm.get("hallucination_rate")
        hall_str = f"  hall={hall:.0f}%" if hall is not None else ""
        print(
            f"  {sl:<18}"
            f"  {m.get('n', 0):>6}"
            f"  {m.get('n_filled', 0):>7}"
            f"  {m.get('n_blank', 0):>6}"
...
def main():
    ...
    # Output dir — resume reuses latest existing folder, fresh run creates timestamped one
    eval_root = base / "eval_results"
    if args.resume:
        existing = sorted([d for d in eval_root.iterdir()
                           if d.is_dir() and d.name.startswith("comparison_")]) \
                   if eval_root.exists() else []
        if existing:
            out_dir = existing[-1]
            print(f"Resuming into existing folder: {out_dir.name}")
        else:
            out_dir = eval_root / f"comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    else:
        out_dir = eval_root / f"comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # Pre-load completed models that are NOT in this run's --models list.
    # This lets you run a single model on H100 (e.g. --models qwen7b --resume)
    # and still get a merged Excel with all previously completed models.
    all_metrics   = []
    requested     = set(args.models)
    if args.resume:
        for mk, cfg in registry.items():
            if mk in requested:
                continue   # will be evaluated below
            rpath = out_dir / f"{mk}_results.json"
            if rpath.exists():
                try:
                    prior = json.loads(rpath.read_text())
                    m = compute_metrics(prior, cfg["label"], cfg["metrics"],
                                        cfg.get("short_label", mk))
                    all_metrics.append(m)
                    print(f"  Pre-loaded: {cfg.get('short_label', mk)}  ({len(prior)} samples)")
                except Exception as e:
                    print(f"  Warning: could not load {mk}_results.json — {e}")
