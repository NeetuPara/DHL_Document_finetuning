...
OUTPUT_DIR = BASE_DIR / cfg_get("paths.output_dir",  "model_output_v3")
...
class TrainingConfig:
    ...
    output_dir:       str  = field(default_factory=lambda: str(BASE_DIR / cfg_get("paths.output_dir", "model_output_v3")))
    ...
    save_total_limit: int | None = field(default_factory=lambda: cfg_get("logging.save_total_limit", None))
...
class VLMCollator:
    ...
    def _resize(self, image: Image.Image) -> Image.Image:
        ...
        # Scale down to max_pixels budget
        if w * h > self.max_pixels:
            scale = (self.max_pixels / (w * h)) ** 0.5
            w = int(w * scale)
            h = int(h * scale)

        # Qwen2.5-VL: ViT uses 14×14 patches merged 2×2 → effective patch = 28×28.
        # visual_tokens = (H/28) × (W/28).  If W or H is not a multiple of 28 the
        # processor snaps to the nearest multiple internally, but the text template
        # was already built with a different token count → IndexError at step N.
        # Fix: snap here so template and vision encoder always agree.
        PATCH = 28
...
def train(method: str = "auto", debug: bool = False, epochs: int = None,
          config_path: str = None, resume_from: str = None):
    ...
    # ── Step 2: Load model ─────────────────────────────────────────────────
    # Instantiate config BEFORE model load so max_seq_length can be passed in.
    # Without this, Unsloth uses the model default (32768 for Qwen2.5-VL-3B)
    # which wastes KV cache memory — 2048 is all we need for our sequences.
    cfg = TrainingConfig()
    ...
    model, processor = FastVisionModel.from_pretrained(
        ...
        max_seq_length=cfg.max_seq_length,   # sets RoPE scaling + KV cache to 2048
    )
    print("Model loaded.")
    ...
    if debug:
        # Quick smoke test: use only 100 training examples
        train_raw = random.sample(train_raw, 100)
        cfg.num_train_epochs = 1
        cfg.save_steps  = 50
        cfg.logging_steps = 10
        print("  DEBUG MODE: using 100 train examples")
    ...
    # Compute step counts for scheduling
    ...
    save_every       = cfg.save_steps   # 500 by default

    print(f"  Steps per epoch  : {steps_per_epoch}")
    print(f"  Save checkpoint  : every {save_every} steps (no eval)")
    ...
    training_args = TrainingArguments(
        ...
        optim                       = "adamw_8bit",  # 8-bit Adam: less VRAM → faster steps

        logging_steps               = cfg.logging_steps,
        eval_strategy               = "no",          # no validation — faster training
        save_strategy               = "steps",
        save_steps                  = save_every,    # checkpoint every 250 steps (from config)
        save_total_limit            = cfg.save_total_limit,

        load_best_model_at_end      = False,

        dataloader_num_workers      = 0,     # 0 = main process only (no multiprocessing)
        dataloader_pin_memory       = False, # Unsloth patches processor at runtime —
        # unpicklable with num_workers > 0 on Windows
    ...
    print(f"  Save checkpoint  : every {save_every} steps  (no validation)")
    print(f"  Optimizer        : adamw_8bit")
    print(f"  Output dir       : {cfg.output_dir}")
    print(f"{'='*60}\n")

    # ── Step 9: TRAIN ─────────────────────────────────────────────────────
    # Auto-find latest checkpoint for resume, or use explicit path
    resume_ckpt = None
    if resume_from:
        if resume_from == "auto":
            ckpts = sorted(
...
if __name__ == "__main__":
    ...
    p.add_argument("--resume",  nargs="?", const="auto", default=None,
                   help="Resume from checkpoint. No value = auto-detect latest. "
                        "Or pass explicit path: --resume model_output_v3/checkpoint-500")
    args = p.parse_args()
    train(method=args.method, debug=args.debug, epochs=args.epochs,
          config_path=args.config, resume_from=args.resume)
